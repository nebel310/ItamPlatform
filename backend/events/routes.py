...
@events_blueprint.route('/cancel_event/<int:event_id>', methods=['POST'])
...
@events_blueprint.route('/register_event/<int:event_id>', methods=['POST'])
def register_event(event_id):
    ...
    if Registration.query.filter_by(user_id=user_id, event_id=event_id).first():
        return jsonify({'error': 'Пользователь уже зарегистрирован на это мероприятие'}), 400

    if len(event.registrations) < event.participant_limit:
        registration = Registration(user_id=user_id, event_id=event_id)
        db.session.add(registration)
        db.session.commit()
        return jsonify({'message': 'Регистрация успешна'}), 201
    else:
        registration = Registration(user_id=user_id, event_id=event_id, status='waiting')
        db.session.add(registration)
        db.session.commit()
        return jsonify({'message': 'Достигнут лимит участников, добавлен в очередь'}), 201


@events_blueprint.route('/unregister_event/<int:event_id>', methods=['POST'])
def unregister_event(event_id):
    ...
    # Проверка очереди
    waiting_registration = Registration.query.filter_by(event_id=event_id, status='waiting').first()
    if waiting_registration:
        waiting_registration.status = 'registered'
        db.session.commit()
        return jsonify({'message': 'Регистрация отменена, первый в очереди зарегистрирован'}), 200

    return jsonify({'message': 'Регистрация отменена'}), 200